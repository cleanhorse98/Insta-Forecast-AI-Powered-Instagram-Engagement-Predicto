#!/usr/bin/env python3
"""
Enhanced XGBoost Training Script with Growth Constraints (views model)
train_views.py - Fixed to ensure predictions always increase
"""
...
def validate_data_for_growth(df, target_col="video_view_count"):
    """
...
def build_features(df):
    ...
    # Add growth rate features if we have views data
    if "video_view_count" in X.columns:
        # Calculate growth rate per day
        X["views_per_day"] = X["video_view_count"] / (X["post_age_days"] + 1e-6)
        X["views_per_hour"] = X["video_view_count"] / (X["post_age_hours"] + 1e-6)
        
        # Log transform for better scaling
        X["log_views"] = np.log1p(X["video_view_count"])
        X["log_views_per_day"] = np.log1p(X["views_per_day"])
        ...
        if "shortcode" in X.columns:
            ...
            for shortcode, group in X.groupby("shortcode"):
                if len(group) > 1:
                    ...
                    rates = group_sorted["views_per_day"].values
                    if len(rates) > 1:
                        accel = np.diff(rates)
                        if len(accel) > 0:
    ...
    if "likes" in X.columns and "video_view_count" in X.columns:
        X["likes_to_views_ratio"] = X["likes"] / (X["video_view_count"] + 1e-6)
        X["engagement_rate_views"] = (X["likes"] + X.get("comments_count", 0)) / (X["video_view_count"] + 1e-6)
    ...
    if "video_view_count" in X.columns:
        X["views_time_interaction"] = X["video_view_count"] * X["day_offset"]
        X["views_log_time_interaction"] = X["log_views"] * X["day_offset_log"]
    ...
    for col in num_cols:
        if col not in ["video_view_count"]:  # Don't fill target column
...
def main():
    ...
    df = validate_data_for_growth(df, target_col="video_view_count")

    # STEP 2: Build features
    X_all = build_features(df)

    # TARGET COLUMN
    if "video_view_count" in X_all.columns:
        y = X_all["video_view_count"]
    else:
        raise ValueError("video_view_count column missing from CSV")

    X = X_all.drop(columns=["video_view_count"])
    ...
    print("\n=== Training Metrics (Views) ===")
    print(metrics)

    # Save
    if args.save_model:
        save_path = os.path.join(
            args.outdir, f"views_model_day{args.day if args.day else 'all'}.joblib"
        )
        joblib.dump(
